...
project <- "Yang_JTK_RUV_run"

options(stringsAsFactors=FALSE)
annot <- read.delim("Yang_JTK_RUV_annot.txt")
data <- read.delim("Yang_JTK_RUV_data.txt")

rownames(data) <- data[,1]
data <- data[,-1]
jtkdist(6, 4)  # 6 total time points, 2 replicates per time point
