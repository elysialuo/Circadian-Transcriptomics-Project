...
project <- "Aviram_JTK_RUV_run"

options(stringsAsFactors=FALSE)
annot <- read.delim("Aviram_JTK_RUV_annot.txt")
data <- read.delim("Aviram_JTK_RUV_data.txt")

rownames(data) <- data[,1]
data <- data[,-1]
jtkdist(12, 4)  # 12 total time points (0-44), 4 replicates per time point
