setwd("/Users/elysialuo/Circadian-Transcriptomics/scripts/R/JTK_Cycle/Mehta_JTK/")
source("../JTK_CYCLE.R")

project <- "Mehta_JTK_RUV_run"

options(stringsAsFactors=FALSE)
annot <- read.delim("Mehta_JTK_RUV_annot.txt")
data <- read.delim("Mehta_JTK_RUV_data.txt")

rownames(data) <- data[,1]
data <- data[,-1]
group.sizes <- c(4,4,4,4,4,4,4,4,3,4,4,4)
jtkdist(length(group.sizes), group.sizes)  # 12 total time points (0-44), 4 replicates per time point

periods <- 12  # looking for rhythms of 24 hours
