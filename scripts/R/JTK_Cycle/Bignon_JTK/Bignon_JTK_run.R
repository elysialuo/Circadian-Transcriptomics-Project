setwd("/Users/elysialuo/Circadian-Transcriptomics/scripts/R/JTK_Cycle/Bignon_JTK/")
source("../JTK_CYCLE.R")

project <- "Bignon_JTK_run"

options(stringsAsFactors=FALSE)
annot <- read.delim("Bignon_JTK_annot.txt")
data <- read.delim("Bignon_JTK_data.txt")

rownames(data) <- data[,1]
data <- data[,-1]
jtkdist(6, 5)  # 6 total time points, 5 replicates per time point
